...
var garage = require("./resources/postGresData").garage;

var markers = require("./resources/postGresData").markers;

var log = require("./resources/postGresData").log;
...
route.get("/getTime", function (req, res) {
  if (req.query.API_KEY == "56ZXRQKLUO2i9P4DQMPH") {
    ...
    log.findAll({
      where: {
        garage: req.query.location,
        time: (_time = {}, _defineProperty(_time, Op.gt, parseInt(req.query.from)), _defineProperty(_time, Op.lt, parseInt(req.query.to)), _time)
      },
      attributes: {
...
route.get("/getMarkers", function (req, res) {
  markers.findAll({}).then(function (result) {
    res.send(JSON.stringify(result));
  });
